-- ===================================================================
-- Blocks a seller (or a BES-key teammate) from placing an order against
-- their own shop. The client already hides/disables every "Add to Bag"
-- entry point for a business the current user can access (addToCart in
-- AppContext.tsx checks accessibleBusinessIds), but per this codebase's
-- established pattern (drop_date embargo in migration_29, proof-of-payment
-- requirement in migration_24), anything only enforced client-side can be
-- bypassed by calling the RPC directly — so place_order() itself must
-- reject it too.
--
-- Safe to run standalone / re-run (idempotent).
-- ===================================================================

-- place_order() re-declared in full (see migration_29_drop_scheduler_hardening.sql)
-- with one addition: reject an order placed against a business the caller
-- owns or is a team member of.
...
begin
  ...
  -- A buyer can't check out against a shop they own or help run — checked
  -- before touching any inventory or coupon so a rejected self-purchase
  -- never leaves a partial side effect behind.
  if exists (
    select 1 from public.businesses
    where businesses.id = v_business_id
    and (
      businesses.seller_id = v_user_id
    ...
    raise exception 'You can''t place an order with your own shop';
  end if;
