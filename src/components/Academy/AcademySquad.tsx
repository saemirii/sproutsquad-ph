...
const ProgressRow: React.FC<{ label: React.ReactNode; value: number; goal: number }> = ({ label, value, goal }) => (
  <div className="space-y-1">
    <div className="flex items-center justify-between text-[11px]">
      <span className="font-bold text-[#3B2F27] flex items-center gap-1">{label}</span>
  ...
  if (!isSupabaseConfigured) {
    return (
      <div className="bg-white p-6 rounded-3xl border border-[#EDE4D8] shadow-xs text-center space-y-2">
        <Icon name="squad-tab" className="w-9 h-9 mx-auto" />
    ...
    <div className="space-y-5">
      <div className="bg-white p-5 rounded-3xl border border-[#EDE4D8] shadow-xs space-y-1">
        <h2 className="text-lg font-black text-[#3B2F27] font-['Nunito',sans-serif] flex items-center gap-1.5">
          <Icon name="squad-tab" className="w-5 h-5" /> Squad Challenges
        </h2>
      ...
      <div className="bg-white rounded-3xl border border-[#EDE4D8] p-6 space-y-4">
        <div className="flex items-center gap-2">
          <Icon name={activeSquadChallenge.icon} className="w-8 h-8" />
          <div>
        ...
        <div className="space-y-3 pt-1">
          <ProgressRow label={<><Icon name="lesson-continue-learning" className="w-3.5 h-3.5" /> Lessons</>} value={squadChallengeProgress.lessons} goal={activeSquadChallenge.goalLessons} />
          <ProgressRow label={<><Icon name="lesson-tip-insight" className="w-3.5 h-3.5" /> Quizzes</>} value={squadChallengeProgress.quizzes} goal={activeSquadChallenge.goalQuizzes} />
          <ProgressRow label={<><Icon name="simulation-retail" className="w-3.5 h-3.5" /> Business Challenges</>} value={squadChallengeProgress.challenges} goal={activeSquadChallenge.goalChallenges} />
        </div>

        {squadChallengeProgress.completed && (
          <div className="bg-[#EBFBF0] border border-[#10B981]/40 rounded-2xl p-4 text-center space-y-2">
            <p className="text-sm font-black text-[#065F46] flex items-center justify-center gap-1.5">
              <Icon name="level-bloom" className="w-5 h-5" /> YOUR SQUAD BLOOMED!
            </p>
            {squadChallengeProgress.claimedByMe ? (
              ...
              <button
                ...
                className="btn-bouncy px-5 py-2.5 rounded-2xl bg-[#207559] hover:bg-[#194E3B] text-white text-xs font-black cursor-pointer inline-flex items-center gap-1"
              >
                Claim +{activeSquadChallenge.rewardXp} XP • +{activeSquadChallenge.rewardSeeds} <Icon name="level-sprout" className="w-3.5 h-3.5" />
              </button>
            ) : (
