  ...
  return (
    <div className="flex flex-col space-y-5 pb-6">
      ...
      <div className="px-4 pt-1">
        {hasNoShopsYet ? (
          <div className="rounded-3xl border border-[#B8E6D5] bg-[#F2FBF7] p-5 space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-xs leading-5 text-[#194E3B]">You don't have a storefront yet. Register one to start adding products, tracking orders, and seeing real analytics.</p>
              </div>
            </div>
            <button
              onClick={() => { playIosTap(); handleCreateFirstShop(); }}
              className="w-full flex items-center justify-center gap-2 rounded-xl bg-[#194E3B] px-3 py-2.5 text-xs font-black text-white"
            >
              <Plus className="w-3.5 h-3.5" />
              Register My Storefront
            </button>
          ...
          <div className="rounded-3xl border border-[#F8BA9E] bg-[#FFF0E8] p-5 space-y-4">
            ...
            <div className="flex gap-2"><input value={besKey} onChange={(event) => setBesKey(event.target.value)} placeholder="Enter BES key" className="min-w-0 flex-1 rounded-xl border border-[#F8BA9E] bg-white px-3 py-2.5 text-xs outline-none focus:ring-2 focus:ring-[#FFD3BA]" /><button onClick={() => { if (unlockBusiness(activeBusiness.id, besKey)) { setAccessError(''); setBesKey(''); } else setAccessError('That BES key is not valid.'); }} className="rounded-xl bg-[#7A341A] px-3 py-2 text-xs font-black text-white flex items-center gap-1.5"><KeyRound className="w-3.5 h-3.5" />Unlock</button></div>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="w-full max-w-md bg-white rounded-t-[32px] p-5 space-y-4 max-h-[75vh] overflow-y-auto animate-in slide-in-from-bottom duration-200">
            ...
            <button
              ...
              className="w-full py-2.5 bg-[#A8D8EA] text-[#1B4E6B] font-extrabold text-xs rounded-2xl flex items-center justify-center gap-2"
            >
              <Plus className="w-4 h-4" />
              <span>Register New Student Storefront</span>
            </button>
          </div>
        </div>
      )}
